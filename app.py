...
def index():
    ...
    if request.method == 'POST':
        date = request.form['date']  # assuming the date is in YYYY-MM-DD
    ...
    cur = db.execute('''select d.entry_date as entry_date, 
                    sum(f.protein) as  protein, sum(f.carbohydrates) as carbohydrates, 
                    sum(f.fat) as fat, sum(f.calories) as calories 
                    from log_date d 
                    join food_date fd on fd.log_date_id = d.id 
...
@app.route('/view/<date>', methods = ['GET','POST'] ) #date is going to be like 20170213
def view(date):
    db = get_db()
